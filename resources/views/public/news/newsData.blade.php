    ...
    @if($loop->first && $allNews->currentPage() == 1)
        <div class="row">
            <div class="col-lg-1 col-md-12 mx-auto" style="max-width: 70px">
                <span class="important-news-date">{{date("d.m", strtotime($news->created_at))}}</span>
            </div>
            <div class="col-lg-6 col-md-12">
                <div class="important-news-title d-inline"> {{$news->title}}</div>
                <div class="important-news-text d-flex mt-3">{{substr(strip_tags($news->text), 0, 200)}}...</div>
                <div><a class="news-link"
                        href="{{route('public.news.show', [class_basename($news), $news->id])}}">@lang('news.more')</a></div>
            </div>
    ...
    @else
        <div class="row">
            ...
            <div class="col-lg-3 col-md-12 position-relative">
                <span class="news-date-small">{{date("F Y", strtotime($news->created_at))}}</span>
            </div>
            <div class="col-lg-5 col-md-12">
                <div class="news-title"> {{$news->title}}</div>
                <div class="mt-5">{{substr(strip_tags($news->text), 0, 110)}}...</div>
                <div><a class="news-link"
                        href="{{route('public.news.show', [class_basename($news), $news->id])}}">@lang('news.more')</a></div>
            </div>
