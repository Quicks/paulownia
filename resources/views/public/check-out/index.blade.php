            ...
            <div class="address-card" v-for='(addresses, index) in this.allAddress'>
                <div class="checkout-address-content" style="display: flex; flex-direction: row; justify-content: space-between; width: 100%;">
                    <label class="radio-container" style="float: right; width: 10%;">
                        <input type="radio" required id="billing[{{$address->id}}]" 
                            name="billing[{{$address->id}}]" value="{{$address->id}}">
                        <span class="checkmark"></span>
                    </label>

                    <ul class="address-card-list" style="float: right; width: 85%;">
                        <li class="mb-3">
                            <b>{{ auth('customer')->user()->first_name }} {{ auth('customer')->user()->last_name }},</b>
                        </li>
                        <li class="mb-2">{{ $address->address1 }},</li>
                        <li class="mb-2">{{ $address->city }},</li>
                        <li class="mb-2">{{ $address->state }},</li>
                        <li class="mb-2">{{ $address->country }}.</li>
                        <li>
                            <b>{{ __('shop::app.customer.account.address.index.contact') }}</b> : {{$address->phone }}
                        </li>
                    </ul>
                </div>
            </div>
