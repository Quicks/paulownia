    ...
    <div class="tab-pane fade show active" id="main-form" role="tabpanel" aria-labelledby="main-form">
        <div class="form-group {{ $errors->has('name') ? 'has-error' : ''}}">
            ...
            <input class="form-control" name="name" type="text" id="name"
                   value="{{ isset($article->name) ? $article->name : ''}}" required>
        ...
        <div class="form-group {{ $errors->has('active') ? 'has-error' : ''}}">
            ...
            <div class="radio">
                <label><input name="active" type="radio"
                              value="1" {{ (isset($article) && 1 == $article->active) ? 'checked' : '' }}> Yes</label>
            </div>
            <div class="radio">
                <label><input name="active" type="radio"
                              value="0" @if (isset($article)) {{ (0 == $article->active) ? 'checked' : '' }} @else {{ 'checked' }} @endif>
                    No</label>
            </div>
        ...
        <div class="form-group {{ $errors->has('publish_date') ? 'has-error' : ''}}">
            ...
            <input class="form-control" name="publish_date" type="date" id="publish_date"
                   value="{{ isset($article->publish_date) ? $article->publish_date : ''}}" required>
        ...
        <div class="tab-pane fade" id={{$locale}} role="tabpanel"
            ...
            <div class="border p-4 mb-4 bg-light rounded">
                @include('admin.multi_lang_inputs.text_input', [
                        'item' => isset($article) ? $article : null, 'itemProperty' => 'title'])

                @include('admin.multi_lang_inputs.text_area', [
                        'item' => isset($article) ? $article : null, 'itemProperty' => 'text'])

                @include('admin.multi_lang_inputs.text_input', [
                        'item' => isset($article) ? $article : null, 'itemProperty' => 'keywords',
