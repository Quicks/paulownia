   ...
   function Plugin(element, options)
   {
     ...
     Plugin.prototype = {
       ...
       init: function()
       {
           ...
           this.startEditListener($('.dd3-content .item-edit'));
           this.endEditListener($('.dd3-content .dd-edit-box .save-btn'));
         ...
         addNewListItemListener: function(el, parent) {
           ...
           el.on('click', function(e) {
             $('.' + opt.rootClass).find('.' + opt.listClass).first().append(_this.createNewListItem());
           })
         },
         startEditListener: function(el) {
           el.on('click', function(event) {
             var e = $(this).parents('.dd3-content').find('span');
             $(this).parents('.dd3-content').first().addClass('edited')
         ...
         endEditListener: function(el) {
           ...
           el.on('click', function(e) {
              $(this).parents('.dd3-content').first().removeClass('edited')
              var spn = $(e.target).parents('.dd3-content').children('span').first(),
              removeBtn = $(this).parent().find('.item-remove');
              removeBtn.slideToggle();

              spn.text($(this).parents('.dd3-content').find('input[name="'+ $("html").attr("lang") + '"]').val());
              ...
              $(e.target).parents('.dd3-content').children('.dd-edit-box').first().slideToggle(50, function() {
                ...
                editBtn = $(e.target).parents('.dd3-content').find('.item-edit').first(),
                ...
                spn.add(editBtn).slideToggle(100);
              });
              $(spn).show()
              if(opt.onEndEdit){
                opt.onEndEdit($(e.target).parents('.dd3-content').children('.dd-edit-box'))
              }
           })
          },
           createNewListItem: function(id, title, http, translations, obj) {
             var title = typeof title !== 'undefined' ? title : 'Item ' + id,
             http = typeof http !== 'undefined' ? http : '',
             el = this.el,
             opt = this.options,
             blueprint = el.find('.dd-item-blueprint').first().clone();
             position = obj && obj.position || this.getHighestPosition()
             
             blueprint.data('position', position);
             blueprint.data('id', id);
             blueprint.attr('class', opt.itemClass);
             blueprint.find('span').first().text(title);
             if(translations){
               translations.forEach(function(translation){
                blueprint.find('input[name="'+ translation.locale +'"]').first().val(translation.title);
               })
               
             }
             
             blueprint.data('title', title);
             blueprint.find('input[name="link"]').first().val(http);
             ...
             this.startEditListener(blueprint.find('.item-edit').first());
            //  this.endEditListener(blueprint.find('.dd-edit-box input'));
             this.endEditListener(blueprint.find('.dd-edit-box .save-btn').first());

             return blueprint;
           },
           getHighestPosition: function() {
             return $('.dd-list').first().children('.dd-item').length
           },
           serialize: function()
           {
             var data,
         ...
         PublicPlugin.prototype = {
           ...
           parseJson: function(data, override) {
             this._plugin.data = data || null, override = override || false;
             this._plugin.deserialize(this._plugin.data, override);
             return this;
           },
           toJson: function() {
